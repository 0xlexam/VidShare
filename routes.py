...
def upload_video_file():
    ...
    video_file.save(upload_dir_path)
    return jsonify({"message": 'Video uploaded successfully!', "video_path": upload_dir_path})

@app.route('/videos', methods=['GET'])
def list_videos():
    videos_directory_path = os.getenv('VIDEO_UPLOAD_PATH')
    video_filenames = [video_name for video_name in os.listdir(videos_directory_path) if os.path.isfile(os.path.join(videos_directory_path, video_name))]
...
def post_comment():
    ...
    try:
        with open(COMMENT_METADATA_PATH, 'r+') as comments_file:
            ...
            json.dump(existing_comments, comments_file)
    except FileNotFoundError:
        with open(COMMENT_METADATA_PATH, 'w') as comments_file:
            json.dump({target_video_id: [user_comment]}, comments_file)
